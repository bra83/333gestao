...
const App: React.FC = () => {
  ...
  const NavButton = ({ v, icon: Icon, label }: { v: ViewState, icon: any, label: string }) => (
    <button 
      ...
      className={`flex flex-col items-center justify-center w-full h-full transition-all border-t-4 ${view === v ? 'border-vault-amber bg-vault-amber/20' : 'border-transparent opacity-30 grayscale'}`}
    >
      <Icon active={view === v} />
      <span className="text-[10px] font-bold mt-1 uppercase glow-text tracking-tighter">{label}</span>
    </button>
  );

  return (
    <div className="min-h-screen bg-vault-bg text-vault-amber flex flex-col">
      <header className="px-6 py-6 flex justify-between items-center bg-vault-panel border-b-2 border-vault-amber relative z-10">
        <div className="flex items-center gap-5">
          <img 
            src="https://raw.githubusercontent.com/bra83/333gestao/main/logomarca.png" 
            alt="Logo" 
            className="w-16 h-16 object-contain logo-vault" 
          />
          <div>
            <h1 className="text-xl font-black tracking-[.25em] text-vault-amber glow-text leading-none uppercase">SAVEPOINT</h1>
            <p className="text-[10px] opacity-60 font-bold uppercase mt-1 tracking-widest">Vault-Tec OS v18.0.9</p>
          </div>
        </div>
        <button onClick={fetchData} className={`p-4 border-2 border-vault-amber ${loading ? 'animate-spin bg-vault-amber text-black' : 'active:scale-95 text-vault-amber'}`}>
          <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3"><path d="M23 4v6h-6M1 20v-6h6M3.51 9a9 9 0 0114.85-3.36L23 10M1 14l4.64 4.36A9 9 0 0020.49 15"/></svg>
        </button>
      </header>

      <main className="flex-1 p-4 max-w-2xl mx-auto w-full main-content">
        ...
        {view === ViewState.CALCULATOR && <Calculator settings={settings} stock={data.estoque} onSaveSale={async (it, mats, ven, luc) => {
           ...
           showToast("REGISTRO SALVO NO TERMINAL");
        }} />}
        {view === ViewState.INVENTORY && <InventoryView stock={data.estoque} onAddStock={async (n,m,p,pr,c,t)=>{
           ...
           if(!confirm("DELETAR HOLOTAPE DEFINITIVAMENTE?")) return;
        ...
        {view === ViewState.SETTINGS && (
          <SettingsView 
            settings={settings} onSave={async (s)=>{setSettings(s); await apiCall({type:'save_settings', ...s}); showToast("PROTOCOLOS ATUALIZADOS");}}
      ...
      <nav className="fixed bottom-0 left-0 right-0 h-20 bg-vault-panel border-t-2 border-vault-amber flex justify-around items-stretch z-[60] pb-safe shadow-[0_-10px_30px_rgba(0,0,0,0.9)]">
        ...
        <NavButton v={ViewState.SETTINGS} icon={({ active }: any) => <svg width="24" height="24" viewBox="0 0 24 24" fill={active ? "currentColor" : "none"} stroke="currentColor" strokeWidth="2.5"><circle cx="12" cy="12" r="3"/><path d="M19.4 15a1.65 1.65 0 0 0 .33 1.82"/></svg>} label="OS" />
      </nav>

      {toast && <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-vault-amber text-black px-8 py-4 font-black z-[100] text-sm tracking-widest uppercase border-4 border-black shadow-[0_0_60px_rgba(255,182,66,0.7)] animate-pulse">{toast}</div>}
    </div>
  );
};
