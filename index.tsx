  ...
  if (isEditing) {
      return (
        <div className="bg-white p-4 rounded-2xl shadow-lg flex flex-col gap-2 border border-emerald-200 relative z-30">
            ...
            <div className="flex gap-2">
                <div className="flex-1"><input type="number" className="w-full bg-slate-50 border border-slate-200 rounded-lg p-1.5 text-slate-700 text-xs" value={editWeight} onChange={e=>setEditWeight(e.target.value)} placeholder="Peso" /></div>
                <div className="flex-1"><input type="number" className="w-full bg-slate-50 border border-slate-200 rounded-lg p-1.5 text-slate-700 text-xs" value={editPrice} onChange={e=>setEditPrice(e.target.value)} placeholder="Preço" /></div>
            </div>
  ...
  return (
    <div className={`bg-white p-4 rounded-3xl shadow-sm flex flex-col items-center relative overflow-hidden group border-2 transition-all hover:shadow-md ${isLowStock ? 'border-red-200 bg-red-50/50' : 'border-white hover:border-emerald-100'}`}>
      <div className="absolute top-2 left-2 z-30 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
        <button onClick={() => setIsEditing(true)} className="bg-white/90 text-blue-400 p-1.5 rounded-full hover:bg-blue-50 border border-blue-100 shadow-sm"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M17 3a2.828 2.828 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5L17 3z"/></svg></button>
        <button onClick={() => item.id && onDelete(item.id)} className="bg-white/90 text-red-400 p-1.5 rounded-full hover:bg-red-50 border border-red-100 shadow-sm"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/><line x1="10" y1="11" x2="10" y2="17"/><line x1="14" y1="11" x2="14" y2="17"/></svg></button>
      </div>
      <div className="absolute top-2 left-2 z-20 group-hover:opacity-0 transition-opacity"><span className="bg-white/80 backdrop-blur text-slate-500 text-[10px] font-black px-2 py-0.5 rounded-full border border-slate-100 shadow-sm uppercase">{type}</span></div>
      <div className={`absolute top-2 right-2 z-20 flex items-center gap-1 px-1.5 py-0.5 rounded-full text-[10px] font-bold shadow-sm ${isLowStock ? 'bg-red-500 text-white animate-pulse' : 'bg-slate-100 text-slate-600'}`}>{isLowStock && <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><line x1="12" y1="2" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>}{Math.round(percentage)}%</div>
      ...
      <h3 className="text-slate-800 font-bold text-base text-center leading-tight mb-0 truncate w-full">{item.nome.replace(/"[^"]+"/g, '').trim()}</h3>
...
const DashboardView: React.FC<{ data: AppData; onNavigate: (view: ViewState) => void }> = ({ data, onNavigate }) => {
    ...
    <div className="space-y-6 pb-20">
      ...
      <div className={`p-6 rounded-2xl shadow-md border transition-all ${netProfit >= 0 ? 'bg-gradient-to-br from-emerald-50 to-white border-emerald-200' : 'bg-gradient-to-br from-red-50 to-white border-red-200'}`}>
        <div className="flex justify-between items-center mb-1">
           ...
           {netProfit >= 0 ? <span className="bg-emerald-100 text-emerald-600 text-[10px] font-bold px-2 py-0.5 rounded-full">POSITIVO</span> : <span className="bg-red-100 text-red-600 text-[10px] font-bold px-2 py-0.5 rounded-full">NEGATIVO</span>}
        </div>
      ...
      <div className="bg-surface p-5 rounded-2xl border border-emerald-100 shadow-sm h-64">
          ...
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={recentSales}>
              ...
              <Tooltip cursor={{fill: '#ecfdf5'}} contentStyle={{backgroundColor: '#fff', borderColor: '#a7f3d0', borderRadius: '12px', boxShadow: '0 4px 6px -1px rgb(0 0 0 / 0.1)', color: '#334155'}} itemStyle={{color: '#10b981', fontWeight: 'bold'}} />
      ...
      <div className="bg-surface p-6 rounded-2xl border border-emerald-100 shadow-sm">
        ...
        <p className="text-secondary text-sm mb-5">Você tem <strong className="text-primary">{data.estoque.length}</strong> rolos registrados.</p>
        <button onClick={() => onNavigate(ViewState.INVENTORY)} className="w-full bg-slate-100 text-slate-600 py-3 rounded-xl text-sm font-bold hover:bg-slate-200 transition-colors">Gerenciar Estoque</button>
      </div>
    </div>
  );
};
...
const Calculator: React.FC<{
  ...
  const [costs, setCosts] = useState({
    materialTotal: 0, energyTotal: 0, machineTotal: 0, laborTotal: 0, fixedTotal: 0,
    servicesTotal: 0, subtotal: 0, riskValue: 0, totalCost: 0, finalPrice: 0, profit: 0
  });
      ...
      <div className="bg-surface p-5 rounded-2xl shadow-sm border border-emerald-100">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="col-span-1 md:col-span-2">
            ...
            <input type="text" className="w-full bg-emerald-50 border border-emerald-100 rounded-xl p-3 text-slate-700 focus:outline-none focus:border-primary/50 font-semibold" value={itemName} onChange={e => setItemName(e.target.value)} />
          </div>
          <div>
            <label className="block text-xs text-secondary mb-1 font-bold ml-1">FILAMENTO</label>
            <select className="w-full bg-emerald-50 border border-emerald-100 rounded-xl p-3 text-slate-700 focus:outline-none focus:border-primary/50 text-sm" value={selectedFilamentIdx} onChange={e => setSelectedFilamentIdx(Number(e.target.value))}>
              {stock.map((item, idx) => (<option key={idx} value={idx}>{item.nome} {item.marca ? `(${item.marca})` : ''}</option>))}
            </select>
          </div>
          <div>
            <label className="block text-xs text-secondary mb-1 font-bold ml-1">ACABAMENTO</label>
            <select className="w-full bg-emerald-50 border border-emerald-100 rounded-xl p-3 text-slate-700 focus:outline-none focus:border-primary/50 text-sm" value={paintingType} onChange={e => setPaintingType(e.target.value as any)}>
          ...
          <div className="flex gap-4">
             <div className="flex-1">
                ...
                <input type="number" className="w-full bg-emerald-50 border border-emerald-100 rounded-xl p-3 text-primary font-black text-xl focus:outline-none focus:border-primary/50" value={weight || ''} onChange={e => setWeight(Number(e.target.value))} />
             </div>
             <div className="flex-1">
                <label className="block text-xs text-secondary mb-1 font-bold ml-1">TEMPO (h)</label>
                <input type="number" className="w-full bg-emerald-50 border border-emerald-100 rounded-xl p-3 text-primary font-black text-xl focus:outline-none focus:border-primary/50" value={hours || ''} onChange={e => setHours(Number(e.target.value))} />
             </div>
          </div>
        </div>
      </div>
...
}> = ({ stock, onAddStock, onUpdateStock, onDeleteStock }) => {
      ...
      <div className="bg-surface p-5 rounded-2xl border border-emerald-100 shadow-sm">
        ...
        <div className="grid grid-cols-1 gap-3">
            <div className="grid grid-cols-2 gap-2">
               <input className="bg-slate-50 border border-slate-200 rounded-xl p-3 text-slate-700 text-sm focus:border-primary focus:outline-none" placeholder='Nome' value={newItemName} onChange={e => setNewItemName(e.target.value)} />
               <select className="bg-slate-50 border border-slate-200 rounded-xl p-3 text-slate-700 text-sm focus:border-primary focus:outline-none" value={newItemType} onChange={e => setNewItemType(e.target.value)}>
                {MATERIAL_TYPES.map(t => <option key={t} value={t}>{t}</option>)}
              </select>
            </div>
            <div className="grid grid-cols-[1fr_auto] gap-2">
              <input className="bg-slate-50 border border-slate-200 rounded-xl p-3 text-slate-700 text-sm focus:border-primary focus:outline-none" placeholder='Marca' value={newItemBrand} onChange={e => setNewItemBrand(e.target.value)} />
              <div className="relative overflow-hidden rounded-xl border border-slate-200 w-12 h-full"><input type="color" className="absolute -top-2 -left-2 w-16 h-16 p-0 border-none cursor-pointer" value={newItemColor} onChange={e => setNewItemColor(e.target.value)}/></div>
            </div>
            <div className="flex gap-2">
...
const SettingsView: React.FC<{
  ...
  onRetry?: () => void;
}> = ({ settings, onSave, apiUrl, onUrlChange, lastError, onRetry }) => {
  ...
  return (
    <div className="space-y-6 pb-20">
      <div className={`bg-white p-5 rounded-2xl border ${lastError ? 'border-red-200 shadow-red-50' : 'border-green-200 shadow-green-50'} shadow-sm`}>
        ...
        <input className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 text-slate-600 text-xs mb-3 font-mono" value={apiUrl} onChange={e => onUrlChange(e.target.value)} placeholder="https://script.google.com/macros/s/.../exec" />
        {lastError && <div className="bg-red-50 p-3 rounded-xl text-xs text-red-500 mb-3 border border-red-100">{lastError}</div>}
        {onRetry && <button onClick={onRetry} className="w-full bg-slate-100 hover:bg-slate-200 text-slate-600 text-xs py-3 rounded-xl font-bold transition-colors">Testar Conexão / Sincronizar Agora</button>}
      </div>
      <div className="bg-white p-6 rounded-3xl shadow-sm border border-emerald-100">
        ...
        <div className="space-y-8">
           <div className="border-b border-emerald-50 pb-6"><h4 className="text-primary text-xs font-black uppercase tracking-wider mb-4">1. Máquina & Energia</h4><div className="grid grid-cols-2 gap-3">
             ...
             <Input label="Manutenção (R$)" val={formData.manutencaoMensal || 20} onChange={v => handleChange('manutencaoMensal', v)} />
           ...
           <div className="border-b border-emerald-50 pb-6"><h4 className="text-primary text-xs font-black uppercase tracking-wider mb-4">2. Empresa & Custos</h4><div className="col-span-2 mb-2"><Input label="Horas Trab/Mês" val={formData.horasTrab} onChange={v => handleChange('horasTrab', v)} /></div><div className="grid grid-cols-3 gap-2"><Input label="MEI" val={formData.mei} onChange={v => handleChange('mei', v)} /><Input label="Aluguel" val={formData.aluguel} onChange={v => handleChange('aluguel', v)} /><Input label="Outros" val={formData.condominio} onChange={v => handleChange('condominio', v)} /></div></div>
           <div className="border-b border-emerald-50 pb-6"><h4 className="text-primary text-xs font-black uppercase tracking-wider mb-4">3. Processos</h4><div className="grid grid-cols-2 gap-3"><Input label="Sua Hora (R$)" val={formData.valorHoraTrabalho || 25} onChange={v => handleChange('valorHoraTrabalho', v)} /><Input label="Markup" val={formData.markup} step={0.1} onChange={v => handleChange('markup', v)} /></div></div>
...
}> = ({ sales, expenses, onUpdateSale, onDeleteSale, onAddExpense, onUpdateExpense, onDeleteExpense }) => {
        ...
        <div className="space-y-4">
          ...
          {sales.map((sale, idx) => (
            <div key={sale.id || idx} className="bg-white p-5 rounded-3xl border border-emerald-50 shadow-sm relative group hover:shadow-md transition-shadow">
              <div className="flex justify-between items-start mb-2"><div><div className="text-slate-700 font-bold text-lg">{sale.item}</div><div className="text-slate-400 text-xs font-semibold">{sale.data} • {sale.material || 'N/A'} ({sale.peso || 0}g)</div></div>
                 <div className="flex gap-2 opacity-100 sm:opacity-0 group-hover:opacity-100 transition-opacity">
                    <button onClick={() => {if(window.confirm('Apagar?')) onDeleteSale(sale.id)}} className="bg-slate-50 p-2 rounded-full text-red-400 hover:bg-red-50 border border-slate-100"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/><line x1="10" y1="11" x2="10" y2="17"/><line x1="14" y1="11" x2="14" y2="17"/></svg></button>
                 </div>
              </div>
              <div className="flex justify-between items-end border-t border-dashed border-slate-100 pt-3 mt-2"><div className="text-green-600 text-xs font-bold bg-green-100 px-3 py-1 rounded-full">Lucro: R$ {sale.lucro.toFixed(2)}</div><div className="text-slate-700 font-black text-xl">R$ {sale.venda.toFixed(2)}</div></div>
            </div>
          ))}
        </div>
      )}
        ...
        <div className="space-y-4">
          ...
          {expenses.slice().reverse().map((exp, idx) => (
             <div key={exp.id || idx} className="bg-white p-4 rounded-2xl border border-slate-100 flex justify-between items-center group relative shadow-sm hover:shadow-md transition-all">
                <div><div className="text-slate-700 font-bold">{exp.descricao}</div><div className="text-slate-400 text-xs font-semibold">{exp.data}</div></div>
                <div className="text-right"><div className="text-red-400 font-black text-lg">- R$ {exp.valor.toFixed(2)}</div></div>
                <div className="absolute top-2 right-2 flex gap-1 opacity-100 sm:opacity-0 group-hover:opacity-100 transition-opacity bg-white pl-2">
                    <button onClick={() => {if(window.confirm('Apagar?')) onDeleteExpense(exp.id)}} className="bg-slate-50 p-1.5 rounded-lg text-red-400 border border-slate-100"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg></button>
                </div>
             </div>
          ))}
        </div>
...
const App: React.FC = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (!settingsRes.ok) throw new Error('Falha ao ler Config');
      const settingsJson = await settingsRes.json();
      setSettings(settingsJson);
      const dataRes = await fetch(`${targetUrl}?type=read_data&t=${ts}`);
      if (!dataRes.ok) throw new Error('Falha ao ler Dados');
      ...
      const fixId = (arr: any[], prefix: string) => (arr || []).map((item: any, i: number) => ({
        ...
        peso: Number(item.peso)||0, preco: Number(item.preco)||0, venda: Number(item.venda)||0, lucro: Number(item.lucro)||0, valor: Number(item.valor)||0
      }));
      setData({ estoque: fixId(dataJson.estoque, 'st'), vendas: fixId(dataJson.vendas, 've'), gastos: fixId(dataJson.gastos, 'ga') });
      showToast('Sincronizado!');
    } catch (err: any) {
      console.error(err); setLastError('Erro de conexão. Modo Offline.'); setData(MOCK_DATA);
  ...
  const apiCall = async (params: URLSearchParams) => {
    if(!apiUrl) return;
    try { await fetch(`${apiUrl}?${params.toString()}`, { method: 'POST', mode: 'no-cors' }); } catch(e) { console.error(e); }
  };
  ...
  const handleUpdateStock = async (id: string, updates: Partial<StockItem>) => {
    setData(prev => ({ ...prev, estoque: prev.estoque.map(item => item.id === id ? { ...item, ...updates } : item) }));
    const currentItem = data.estoque.find(i => i.id === id);
    if (!currentItem) return;
    const finalItem = { ...currentItem, ...updates };
    const params = new URLSearchParams({ type: 'estoque', action: 'update', id, nome: finalItem.nome || '', marca: finalItem.marca || '', peso: (finalItem.peso || 0).toString(), preco: (finalItem.preco || 0).toString(), cor: finalItem.cor || '', tipo: finalItem.tipo || '' });
    apiCall(params);
  ...
  const handleAddSale = async (item: string, material: string, peso: number, venda: number, lucro: number, stockId?: string) => {
    ...
    const params = new URLSearchParams({ type: 'venda', action: 'create', id, item, material, peso: peso.toString(), venda: venda.toFixed(2), lucro: lucro.toFixed(2) });
    apiCall(params);
    showToast('Venda ok!');
  ...
  const handleUpdateSale = async (id: string, newVal: number, newProfit: number) => {
    ...
    if (sale) {
       const params = new URLSearchParams({ type: 'venda', action: 'update', id, data: sale.data, item: sale.item, material: sale.material, peso: sale.peso.toString(), venda: newVal.toFixed(2), lucro: newProfit.toFixed(2) });
      apiCall(params);
    }
  ...
  const handleAddExpense = async (descricao: string, valor: number, dataStr: string) => {
    ...
    const params = new URLSearchParams({ type: 'gasto', action: 'create', id, data: dataStr, descricao, valor: valor.toString() });
    apiCall(params);
    showToast('Gasto ok!');
  };
  ...
  const handleUpdateExpense = async (id: string, descricao: string, valor: number) => {
    ...
    if (exp) {
      const params = new URLSearchParams({ type: 'gasto', action: 'update', id, data: exp.data, descricao, valor: valor.toString() });
      apiCall(params);
    }
    showToast('Atualizado!');
  };
  ...
  const NavButton = ({ v, icon: Icon, label }: any) => (
  ...
  return (
    <div className="min-h-screen bg-dark text-slate-600 font-sans selection:bg-primary selection:text-white">
      <header className="sticky top-0 bg-white/80 backdrop-blur-md border-b border-emerald-100 p-4 z-20 flex justify-between items-center shadow-sm">
        <div className="flex items-center gap-2"><div className="flex items-center -space-x-1.5 select-none"><span className="text-4xl font-black text-primary tracking-tighter">3</span><span className="text-4xl font-black text-emerald-300 tracking-tighter z-10">3</span><span className="text-4xl font-black text-primary tracking-tighter">3</span></div></div>
      ...
      <main className="p-4 max-w-lg mx-auto">
        ...
        {view === ViewState.SETTINGS && <SettingsView settings={settings} onSave={handleSaveSettings} apiUrl={apiUrl} onUrlChange={val => {setApiUrl(val); localStorage.setItem('APPS_SCRIPT_URL', val);}} lastError={lastError} onRetry={fetchData} />}
      </main>
